 ...
 * @author user
 */
public class EXIST_IN_RETAILER_DB implements  RuleEngineInterface {

     static final Logger logger = LogManager.getLogger(EXIST_IN_RETAILER_DB.class);
     ...
     public String executeRule(RuleEngine ruleEngine)  {
          ...
          try {
               ...
               {
                    result1 = stmt2.executeQuery("select count(imei_esn_meid) from device_retailer_db  where imei_esn_meid='" + ruleEngine.imei + "' ");
                    logger.debug("select count(imei_esn_meid) from device_retailer_db  where imei_esn_meid='" + ruleEngine.imei + "' ");
                     int res2 =  0;
                    try {
                    while (result1.next()) {
          ...
          try {
               switch (ruleEngine.action) {
                    ...
                    case "Reject": {
                         ...
                         String fileString = ruleEngine.fileArray + " , Error Code :CON_RULE_0008, Error Description : IMEI/ESN/MEID is already present in the system  ";
...
////
////                String historyIns = " insert into device_retailer_db (created_on,device_id ,device_type ,  device_status , imei_esn_meid   ) values  ( '" + date + "',  '" + ruleEngine.sNofDevice + "'    , 10 , '" + ruleEngine.imei + "' ) ";
