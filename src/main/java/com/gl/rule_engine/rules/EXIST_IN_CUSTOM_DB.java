 ...
 * @author user
 */
public class EXIST_IN_CUSTOM_DB implements  RuleEngineInterface {
 ...
 Statement stmt2 = null;
        ...
        try {
            ...
            {
                  result1 = stmt2.executeQuery("select count(imei_esn_meid) from device_custom_db  where imei_esn_meid='" + ruleEngine.imei + "' ");
                          logger.debug("select count(imei_esn_meid) from device_custom_db  where imei_esn_meid='" + ruleEngine.imei + "' ");
               int res2 =  0;
                    try {
                    while (result1.next()) {
