 ...
 * @author user
 */
public class EXIST_IN_END_USER_DEVICE_DB implements  RuleEngineInterface {
     ...
     public String executeRule(RuleEngine ruleEngine)  {
          ...
          try {
               ...
               {
                    result1 = stmt2.executeQuery("select count(imei_esn_meid) from end_user_device_db  where imei_esn_meid='" + ruleEngine.imei + "' ");
                    logger.debug("select count(imei_esn_meid) from end_user_device_db  where imei_esn_meid='" + ruleEngine.imei + "' ");
                    int res2 = 0;
                    try {
                         while (result1.next()) {
